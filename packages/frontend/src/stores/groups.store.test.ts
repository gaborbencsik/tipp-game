...
describe('groups.store', () => {
  ...
  it('createGroup() → new group appended to list', async () => {
    ...
    const store = useGroupsStore()
    await store.fetchMyGroups()
    await store.createGroup({ name: 'Munkások', leagueIds: ['l-1'] })
  ...
  it('createGroup() → returns created group', async () => {
    ...
    const store = useGroupsStore()
    const result = await store.createGroup({ name: 'Barátok', leagueIds: ['l-1'] })
  ...
  it('createGroup() error → throws', async () => {
    ...
    const store = useGroupsStore()
    await expect(store.createGroup({ name: 'Hatodik', leagueIds: ['l-1'] })).rejects.toThrow('Maximum number of created groups reached')
  })

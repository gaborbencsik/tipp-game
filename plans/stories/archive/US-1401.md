...
- [ ] A banner magassaga fix: **mobilon 160px**, **tableten 180px**, **desktoppon 200px**
...
- [ ] **Tablet** (640px-1024px): banner magassag 180px, szoveg 16px/14px, padding 16px
- [ ] **Desktop** (> 1024px): banner magassag 200px, szoveg 18px/14px, padding 24px
...
- [ ] A kepek `loading="eager"` + `fetchpriority="high"` attributummal toltodnek (above-the-fold elem)
- [ ] A banner kontener fix magassaggal rendelkezik (nincs CLS — Cumulative Layout Shift)
- [ ] A gradient hatter az alapertelmezett allapot; a kep betoltesekor fade-in-nel jelenik meg (`transition-opacity duration-300`)
...
A VB 2026 16 stadionjához statikus képek kerülnek a repository-ba (`packages/frontend/public/venues/`). A fájlnevek slug formátumúak (pl. `metlife-stadium.jpg`).
...
Ajanlott merete: **1200x400px** (3:1 arany, optimalizalt webre).

**A `venues.imageUrl` mező a relatív elérési utat tárolja** (pl. `/venues/metlife-stadium.jpg`), nem külső CDN linket. Ez biztosítja, hogy a képek nem törhetnek el külső szolgáltató változás miatt.
...
<!-- VenueBanner.vue -->
<template>
  <div v-if="venue" class="relative w-full rounded-lg overflow-hidden h-40 sm:h-[180px] lg:h-[200px]">
    <!-- Fallback gradient (always visible as base) -->
...
2. **Seed:** A 16 stadion kepeihez a `/venues/<slug>.jpg` formatumu relativ URL-eket hasznalj. A kepek a `packages/frontend/public/venues/` mappaban lesznek (1200x400px, jogmentes forrasbol).
3. **Teszt:** A `VenueBanner` komponenshez kulon teszt fajl (`VenueBanner.spec.ts`), ne a `MatchDetailView` tesztjebe keverve
4. **Tailwind v4:** A responsive osztalyok (`sm:`, `lg:`) a Tailwind v4-ben valtozatlanul mukodnek. A `h-40` = 160px, `h-[180px]` = 180px, `h-[200px]` = 200px.
5. **Kep optimalizalas:** A kepeket 1200x400px-re kicsinyitsd es JPEG quality 80%-ra kompressald mielott a repo-ba kerul.
